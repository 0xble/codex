...
#[test]
fn terminal_capabilities_disable_fragile_features_for_mosh() {
    ...
    let capabilities = detect_terminal_capabilities(Some(TerminalTransport::Mosh), &info);

    assert_eq!(
        capabilities,
        TerminalCapabilities {
            supports_alternate_scroll: false,
            supports_bracketed_paste: false,
            supports_focus_change: false,
...
#[test]
fn terminal_capabilities_disable_synchronized_updates_in_tmux() {
    let info = terminal_info(
        TerminalName::Ghostty,
        Some("Ghostty"),
        None,
    ...
    let capabilities = detect_terminal_capabilities(None, &info);
...
#[test]
fn terminal_capabilities_reflect_terminal_specific_keyboard_support() {
    let supported = terminal_info(TerminalName::Ghostty, Some("Ghostty"), None, None, None);
    let unknown = terminal_info(TerminalName::Unknown, None, None, None, None);

    assert!(detect_terminal_capabilities(None, &supported).supports_keyboard_enhancement);
    assert!(!detect_terminal_capabilities(None, &unknown).supports_keyboard_enhancement);
}
